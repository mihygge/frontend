...
const StaffDetails = (props) => {
    ...
    return (
        <StaffDetailsPartial {...props} FormValues={FormValues} />
    )
}
        ...
        <div className="form-in-new-care">
            <Form noValidate onSubmit={handleSubmit}>
                <div className="two-inputs-holder">
                    <FormInput type="text" labelText="Board Members" name="board_member" placeholder="Enter board member details here (Ex - Kim, John)"/>
                </div>
                <div className="two-inputs-holder">
                    <FormInput type="text" labelText="Owner" name={STAFF_ROLE.OWNER} placeholder="Enter owner name here" required/>
...
const StaffDetailsPartial = withFormik({
    ...
    handleSubmit: (values) => {
        ...
        const loadingMsg = !isUpdate ? 'Adding staff details...' : 'Updating staff deatils...';
        const successMsg = !isUpdate ? 'Staff details added...'  : 'Staff Details updated successfully.';

        const { hide: hideLoading } = cogoToast.loading(loadingMsg, { hideAfter: 0 });

        UpdateStaffDetails(care_id, req_body).then(() => {
            hideLoading();
